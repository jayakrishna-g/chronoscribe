...
from modules.room.model import Room, TranscriptInstance
...
async def get_owner_rooms(owner):
    rooms = room_coll.find({"owner": owner}, {"name": 1, "description": 1, "room_id": 1, "_id": 0})
    return await rooms.to_list(length=1000)



asyncio.get_event_loop().create_task(sync_rooms())


def create_broadcast_message(service, message):
    return {"status": True, "type": "broadcast", "body": { "service": service,  "message": message}}
...
async def get_room(room_id):
    ...
    if room:
        room_obj = Room(room["name"], room["description"], room["owner"], room["room_id"])
        room_obj.transcript = list(map(lambda x: TranscriptInstance(transcript_content=x[1], transcript_index=x[0]), enumerate(room["transcript"])))
...
async def save_room(room : Room):
    ...
    if len(room.summaries) > 0 and room.has_newSummary():
        await connection_manager.broadcast(room.room_id, create_broadcast_message("summary", room.summaries[-1]))
    room_dict = room.db_dict()
    room_id = room_dict["room_id"]
    await room_coll.update_one({"room_id": room_id}, {"$set": room_dict}, upsert=True)
    return room_id

async def create_room(name, description, owner):
...
class RoomService:
    async def transcript(self, room_id, websocket, data, connection_manager):
        if "transcript_content" in data and "transcript_index" in data:
            message = {"transcript_content": data["transcript_content"], "transcript_index": data["transcript_index"]}
            room = await get_room(room_id)
            room.save_transcript(TranscriptInstance(**message))
            await connection_manager.broadcast(room_id, create_broadcast_message("transcript", message))

    async def emoji(self, room_id, websocket, data, connection_manager):
        if "emoji" in data:
            message = {"emoji": data["emoji"]}
            await connection_manager.broadcast(room_id, create_broadcast_message("emoji", message))
    ...
    async def quick_question(self, room_id, websocket, data, connection_manager):
        if "question" in data:
            ...
            await connection_manager.broadcast(room_id, create_broadcast_message("quick_question", message))

    async def quick_question_answer(self, room_id, websocket, data, connection_manager):
        if "answer" in data:
            message = {"answer": data["answer"]}
            await connection_manager.broadcast(room_id, create_broadcast_message("quick_question_answer", message))

    async def question(self, room_id, websocket, data, connection_manager):
        if "question" in data:
            message = {"question": data["question"]}
            await connection_manager.broadcast(room_id, create_broadcast_message("question", message))
...
async def handle_room_socket(websocket, data, room_id, connection_manager):
    ...
    await room_service.handle(websocket, data, room_id, connection_manager)
